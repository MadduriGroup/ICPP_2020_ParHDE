...
int main(int argc, char *argv[]) {
    ...
    vector< vector<pair<unsigned int, int32_t> > > AdjVector;
    ...
    while (fgets(line, sizeof(line), inFilePtr) != NULL) {
        if (line[0] == '%') {
            ...
            if (line[1] == '%') {
                ...
                if (found == string::npos) {
                    ...
                    headerWord = string("integer");
                    found = str.find(headerWord);
                    if (found != string::npos) {
                        cout << "Matrix has integer weights, using them" << endl;
                        patternFormat = 2;
                    }
                }
            }
        ...
        if (lineCount == 1) {
            ...
            if (patternFormat == 1) {
                sscanf(line, "%u %u\n", &u, &v);
            } else if(patternFormat == 0) {
                double w;
                sscanf(line, "%u %u %lf\n", &u, &v, &w);
            }
            else if(patternFormat == 2) {
                sscanf(line, "%u %u %d\n", &u, &v, &w);
            }
            // cout << u << " " << v << endl;
            ...
            if (symmetricFormat) {
                if (u != v)  {
                    ...
                    AdjVector[u-1].push_back(make_pair(v-1,w));
                    AdjVector[v-1].push_back(make_pair(u-1,w));
                }
            } else {
                u += N2;
                AdjVector[u-1].push_back(make_pair(v-1,w));
    ...
    for (long i=0; i<N; i++)  {
        vector<pair<unsigned int, int32_t> > AdjVectorNoDup;
        if (AdjVector[i].size() > 1) {
            pair<unsigned int, int32_t> prevVtx = AdjVector[i][0];
            AdjVectorNoDup.push_back(prevVtx);
            for (unsigned int len = 1; len<AdjVector[i].size(); len++) {
                pair<unsigned int, int32_t> currVtx = AdjVector[i][len];
                if (currVtx.first != prevVtx.first) {
                    prevVtx = currVtx;
                    AdjVectorNoDup.push_back(prevVtx);
                }
            }
    ...
    for (long i=0; i<N; i++) {
        ...
        while (currentPosS != visitedCount) {
            ...
            for (unsigned int j=0; j<AdjVector[currentVert].size(); j++) {
                unsigned int v = AdjVector[currentVert][j].first;
                if (CompID[v] == 0) {
                    S[visitedCount++] = v;
                    CompID[v] = numComps;
                }
    ...
    */
    ...
    for (long i=0; i<Nold; i++) {
        if (CompID[i] != 0) {
            ...
            for (size_t j=0; j<AdjVector[i].size(); j++) {
                unsigned int v = CompID[AdjVector[i][j].first]-1;
                unsigned int adjPos = num_edges[u] + j;
                assert(((long) adjPos) < M);
                adj[adjPos] = v;
                weights[adjPos] = AdjVector[i][j].second;
            }
        }
    }
    ...
    if(patternFormat == 2)
        fwrite(&weights[0], sizeof(int32_t), M, writeBinaryPtr);
