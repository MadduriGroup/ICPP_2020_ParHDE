...
*/
    ...
    template<typename T>
    bool inline compare_and_swap(T &x, const T &old_val, const T &new_val) 
    {
      return __sync_bool_compare_and_swap(&x, old_val, new_val);
    }

    template<>
    bool inline compare_and_swap(float &x, const float &old_val, const float &new_val) 
    {
    ...
    template<>
    bool inline compare_and_swap(double &x, const double &old_val, const double &new_val) 
    {
    ...
    int32_t inline fetch_and_add(int32_t &x, int32_t inc) 
    {
      return atomic_add_32_nv((volatile uint32_t*) &x, inc) - inc;
    }

    int64_t inline fetch_and_add(int64_t &x, int64_t inc) 
    {
      return atomic_add_64_nv((volatile uint64_t*) &x, inc) - inc;
    }

    uint32_t inline fetch_and_add(uint32_t &x, uint32_t inc) 
    {
      return atomic_add_32_nv((volatile uint32_t*) &x, inc) - inc;
    }

    uint64_t inline fetch_and_add(uint64_t &x, uint64_t inc) 
    {
      return atomic_add_64_nv((volatile uint64_t*) &x, inc) - inc;
    }

    bool inline compare_and_swap(int32_t &x, const int32_t &old_val, const int32_t &new_val) 
    {
      return old_val == atomic_cas_32((volatile uint32_t*) &x, old_val, new_val);
    }

    bool inline compare_and_swap(int64_t &x, const int64_t &old_val, const int64_t &new_val) 
    {
      return old_val == atomic_cas_64((volatile uint64_t*) &x, old_val, new_val);
    }

    bool inline compare_and_swap(uint32_t &x, const uint32_t &old_val, const uint32_t &new_val) 
    {
      return old_val == atomic_cas_32((volatile uint32_t*) &x, old_val, new_val);
    }

    bool inline compare_and_swap(uint64_t &x, const uint64_t &old_val, const uint64_t &new_val) 
    {
      return old_val == atomic_cas_64((volatile uint64_t*) &x, old_val, new_val);
    }
  ...
  template<typename T, typename U>
  T inline fetch_and_add(T &x, U inc) 
  {
    T orig_val = x;
    x += inc;
    return orig_val;
