...
void inline PrintLabel(const std::string &label, const std::string &val) 
{
  printf("%-21s%7s\n", (label + ":").c_str(), val.c_str());
}

void inline PrintTime(const std::string &s, double seconds) 
{
  printf("%-21s%3.5lf\n", (s + ":").c_str(), seconds);
}

void inline PrintStep(const std::string &s, int64_t count) 
{
  printf("%-14s%14" PRId64 "\n", (s + ":").c_str(), count);
}

void inline PrintStep(int step, double seconds, int64_t count = -1) 
{
  if (count != -1)
...
void inline PrintStep(const std::string &s, double seconds, int64_t count = -1) 
{
  if (count != -1)
